...
class LocationService{
    ...
    _init(){
        this.dbRef.on('child_added', (data) => {
            this.adapter.log.info('loation update received for key=' + data.key + ' data: ' + JSON.stringify(data.val()));
    ...
    saveLocation(id, data){
        ...
        for(let deviceId in data){
            let id = 'iogo.0.' + deviceId + '.locations.' + locationId;
            this.adapter.setObjectNotExists(id, {
                ...
                if (!err && obj) {
                    ...
                    this.adapter.setState(id, data[id]);
                }
            });
        }
    }
