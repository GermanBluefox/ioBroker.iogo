...
class SyncService{
    constructor(adapter, firestore, database, uid, name){
        ...
        this.colRef = this.firestore.collection("users").doc(this.uid).collection(name + 's');
    ...
    syncChecksum(){
        this.colRefMeta.doc(this.name + 'ChecksumMap').set(super.checksumMap);
    }
    ...
    deleteObject(id){
        ...
        this.adapter.log.debug('object (' + name + ') ' + id + ' removed successfully');
        this.syncChecksum();
    }
    ...
    syncObject(id, object){
        ...
        this.colRef.doc(node).set(object)
            .then(function() {
                this.adapter.log.debug('object (host) ' + id + ' saved successfully');
            })
            .catch(function(error) {
                this.adapter.log.error(error);
            });
        super.checksumMap[node] = object.checksum;
        this.syncChecksum();
    }
    ...
    syncObjectList(objectList){
        ...
        for(let x in super.checksumMap){
        ...
        if(val !== null){
            this.database.ref(this.name + 's/' + this.uid + '/' + node + '/' + attr).set(val, (error) => {
                if (error) {
                    ...
                    this.adapter.log.debug('host ' + id + ' updated successfully');
                }
            });
        }
    }
    ...
    syncStateList(stateList){
        this.database.ref(this.name + 's/' + uid).set(stateList, function(error) {
            if (error) {
                this.adapter.log.error(error);
            } else {
